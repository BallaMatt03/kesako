<script type="text/javascript">
	var wsUri = "ws://192.168.1.102:9000/demo/server.php"; 	
	...
	function receiveMessage(author, message) {
		...
		$("#profil-img").attr('src', fileExists(author));
		$("#messages").css('visibility','visible');
		$("#author").text(author+"");
		$("#income-message").text(message+"");
	}
	
	function fileExists (author) {
				
		var response = jQuery.ajax({
			url:  "../img/joueurs/"+author+".jpg",
			type: 'HEAD',
			async: false
		}).status;	
